var DistanceTravelledHubBoard = React.createClass({
        displayName: "DistanceTravelledHubBoard",
      ...
      var GraphBox = React.createClass({
        ...
        componentWillReceiveProps: function componentWillReceiveProps(nextProps){
          ...
          if(nextProps.resolution=="today"){
            today_date = new Date();
            dates_array=[]; 
            distance_travelled_hub_array=[0];           
          ...
          query.find({
            success: function(results) {              
              ...
              for(var j=0;j<dates_array.length;j++){              
                for(var k=0;k<results.length;k++){
                  if(results[k].get("orderdatetime").substr(0,10)==dates_array[j][0]){
                    distance_travelled_hub_array[j] += results[k].get("distancetraveled");
                  }                              
                }                
              }
              ...
              {
                x: dates_array,
                y: distance_travelled_hub_array,
                type: 'bar'
              }
                ]});
          },
        ...
        render: function render() {             
          ...
          var titlee = "Average distance travelled for hub "+this.props.hub+" for "+this.props.resolution;
          var layout = {
            title: titlee
          };
